...
V5 起，已完成辩论可以创建追问子辩论。子辩论复用原 `article_id`，使用 `parent_debate_id` 关联直接父辩论，并把当前问题保存在 `follow_up_question`。`parent_context_snapshot` 只保存裁判结论、判定依据、关键分歧、可信部分和存疑部分等裁剪信息，不保存父辩论全部 Agent 消息。删除父辩论时保留子辩论和上下文快照，并将直接父关联置空。
...
V5 增加响应式共享导航和页面层级返回入口。首页提供继续工作区和五步辩论流程；已完成辩论详情页可以提交新问题，通过 `POST /api/debates/{debate_id}/follow-ups` 创建独立子辩论并跳转到新详情页。
...
- 继续追问只传递字段化父结果快照；标量、数组数量和数组项长度均受限，禁止无限拼接完整历史消息或祖先链。
- 文章与话题 Agent prompt 明确把父裁判结论视为待检验背景，不直接继承上一轮胜负判断。
