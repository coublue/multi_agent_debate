...
![多 Agent 分析工作台](./docs/images/readme/01-home-workspace.png)

首页集中展示文章辩论、快速话题辩论和最近任务。用户可以从统一入口发起分析，也可以快速返回正在运行或最近完成的辩论。

### 结构化辩论流程

![结构化辩论流程](./docs/images/readme/02-debate-flow.png)

系统按照“输入文章或话题 → 提取关注问题 → 主持人定义边界 → 正反方多轮交锋 → 裁判形成报告”的流程组织多个 Agent，完整保留分析链路和阶段结果。

### 历史辩论管理

![历史辩论管理](./docs/images/readme/04-debate-history.png)

历史辩论页面集中展示任务状态、可信度评分、裁判结论和创建时间，并支持搜索、状态筛选、排序、详情查看与记录删除。

### 辩论详情与继续追问

![辩论详情与继续追问](./docs/images/readme/06-debate-detail.png)

辩论详情页展示完成状态、阶段进度、正反方核心观点和裁判结论。对于已完成的辩论，用户可以基于当前结果提出新问题，创建独立的追问子辩论，同时保留原始结果。

### 结构化裁判报告

![结构化裁判报告](./docs/images/readme/07-judge-report.png)

裁判 Agent 综合双方论证，输出结论方、可信度、双方最强论点、可信与存疑部分、关键分歧、判定依据及最终总结，便于复核并导出 Markdown 报告。
